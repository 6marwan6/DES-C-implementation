...
static const int shift_left[16] = {
1, 1, 2, 2,
2, 2, 2, 2,
1, 2, 2, 2,
2, 2, 2, 1
...
uint64_t permute(uint64_t input, const int *table, int size, int out_bits) {
    ...
    /// 2) usage example :   uint64_t result = permute(input, permuted_choice_2, 56, 48);
    /// 3) input is the input bits to be permuted 
    /// 4) table is the permutation table : ex : initial_permutation
    /// 5) size is the size of input in bits ( ex : 64 )
    /// 6) out_bits is the size of output in bits ( ex : 64 for initial_permutation )
    /// 7) Returns the permuted output as uint64_t
    ...
    for (int i = 0; i < out_bits; i++) {
        ...
        /// start filling the last (least significant) bit of output then shift left to make room for the next bit to be added in the least significant bit again
        output <<= 1;

        /// input >> (size - src_pos - 1)   to shift the bit we want to permute to the least significant bit position 
        /// & 1 -> isolates only that bit 
        /// ORing that bit with the output to put it in the least significant bit position of output
